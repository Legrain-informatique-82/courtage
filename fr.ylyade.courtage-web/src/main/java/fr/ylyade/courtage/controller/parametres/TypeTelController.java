...
public class TypeTelController implements Serializable {  
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3339516078041348530L;
	...
	public void actEnregistrer(ActionEvent actionEvent){
		try {
			...
			if(nouveau.getId()==null || taTTelService.findById(nouveau.getId()) == null){
				...
				taTTel = taTTelService.merge(taTTel, ITaTTelServiceRemote.validationContext);
			...
			if(modeEcranDefaut!=null && modeEcranDefaut.equals(C_DIALOG)) {
				RequestContext.getCurrentInstance().closeDialog(taTTel);
			}
		} catch(Exception e) {
			e=ThrowableExceptionLgr.renvoieCauseRuntimeException(e);
			e.printStackTrace();
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "TypeTel", e.getMessage()));
		}
	}
	...
	public void actSupprimer(){
		TaTTel truc = new TaTTel();
		try {
			if(selection!=null && selection.getId()!=null){
				truc = taTTelService.findById(selection.getId());
			}

			taTTelService.remove(truc);
			...
			if(ConstWeb.DEBUG) {
				FacesContext context = FacesContext.getCurrentInstance();  
				context.addMessage(null, new FacesMessage("Type telephone", "actSupprimer"));
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			FacesContext context = FacesContext.getCurrentInstance();  
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Type telephone", e.getCause().getCause().getCause().getCause().getMessage()));
		}
	}
		...
		try {
			if(nomChamp.equals(Const.C_CODE_T_TEL)) {
				...
				if(changement && modeEcran.dataSetEnModeModification()) {
					FacesContext context = FacesContext.getCurrentInstance();  
					context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Type de Telephone", Const.C_MESSAGE_CHANGEMENT_CODE));
				}
			}
			return false;
	...
	public void actDialogTypes(ActionEvent actionEvent) {
        ...
        RequestContext.getCurrentInstance().openDialog("parametres/dialog_type_tel", options, params);
	}
