...
public class TypeAdresseController implements Serializable{  
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1674141896659816479L;
	private List<TaTAdresseDTO> values; 
	private List<TaTAdresseDTO> filteredValues; 
	private TaTAdresseDTO nouveau ;
	private TaTAdresseDTO selection ;
	
	private TaTAdresse taTAdr = new TaTAdresse();
	...
	private @EJB ITaTAdresseServiceRemote taTAdrService;
	
	private LgrDozerMapper<TaTAdresseDTO,TaTAdresse> mapperUIToModel  = new LgrDozerMapper<TaTAdresseDTO,TaTAdresse>();
	private LgrDozerMapper<TaTAdresse,TaTAdresseDTO> mapperModelToUI  = new LgrDozerMapper<TaTAdresse,TaTAdresseDTO>();
	...
	public void actEnregistrer(ActionEvent actionEvent){
		try {
			TaTAdresseDTO retour = null;
			taTAdr=new TaTAdresse();
			if(nouveau.getId()==null || taTAdrService.findById(nouveau.getId()) == null){
				mapperUIToModel.map(nouveau, taTAdr);
				taTAdr = taTAdrService.merge(taTAdr, ITaTAdresseServiceRemote.validationContext);
	...
	public void validateObject(FacesContext context, UIComponent component, Object value) throws ValidatorException {
		...
		try {
			...
			Set<ConstraintViolation<TaTAdresseDTO>> violations = factory.getValidator().validateValue(TaTAdresseDTO.class,nomChamp,value);
			if (violations.size() > 0) {
				messageComplet = "Erreur de validation : ";
				for (ConstraintViolation<TaTAdresseDTO> cv : violations) {
	...
	public boolean validateUIField(String nomChamp,Object value) {
		try {
			if(nomChamp.equals(Const.C_CODE_T_ADR)) {
				...
				if(selection.getCodeTAdresse()!=null && value!=null && !selection.getCodeTAdresse().equals("")) {
					if(value instanceof TaTAdresse)
						changement=((TaTAdresse) value).getCodeTAdresse().equals(selection.getCodeTAdresse());
					else if(value instanceof String)
					changement=!value.equals(selection.getCodeTAdresse());
				}
	...
	public void actDialogTypes(ActionEvent actionEvent) {
        ...
        RequestContext.getCurrentInstance().openDialog("parametres/dialog_type_adresse", options, params);
	}
	
	public void handleReturnDialogTypes(SelectEvent event) {
		if(event!=null && event.getObject()!=null) {
			taTAdr = (TaTAdresse) event.getObject();
		}
		refresh();
	}
...
//		modeEcran.setMode(EnumModeObjet.C_MO_EDITION);
	...
	public List<TaTAdresseDTO> getValues(){  
		return values;
	}

	public TaTAdresseDTO getNouveau() {
		return nouveau;
	}

	public void setNouveau(TaTAdresseDTO newTaTAdresse) {
		this.nouveau = newTaTAdresse;
	}

	public TaTAdresseDTO getSelection() {
		return selection;
	}

	public void setSelection(TaTAdresseDTO selectedTaTAdresse) {
		this.selection = selectedTaTAdresse;
	}

	public void setValues(List<TaTAdresseDTO> values) {
		this.values = values;
	}

	public List<TaTAdresseDTO> getFilteredValues() {
		return filteredValues;
	}

	public void setFilteredValues(List<TaTAdresseDTO> filteredValues) {
