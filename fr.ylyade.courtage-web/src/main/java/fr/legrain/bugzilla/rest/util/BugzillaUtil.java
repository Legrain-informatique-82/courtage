...
import fr.legrain.bugzilla.rest.model.CommentSearch;
...
public class BugzillaUtil {
	...
	private String apiKey; 		//"zpV28PVz133uhN8aLHaDKOtRThw3nxc8UMExC6JW";
	private String login;		// = "nicolas@legrain.fr";
	private String password; 	// = "lgr006";
	private String bugzillaURL; //"http://bugs.legrain.fr"
...
//		Client client = ClientBuilder.newClient().register(new Authenticator(login, password));
	...
	public List<Bug> findBugs(String product, String userId) {

		service = client.target(restURL+"/bug?"+apiKeyParamString+"&product="+product+"&creator="+userId);
	...
	public List<Comment> findBugComment(int bugID) {
		...
		CommentSearch b = service.request(MediaType.APPLICATION_JSON).get(CommentSearch.class);
		return b.getResult().getComments();
	}
...
//		mapper.setAnnotationIntrospector(introspector);
	...
	public class Authenticator implements ClientRequestFilter {
	 ...
	 static public Date bugzillaStringDateToDate(String dateBugzilla) {
    	//exemple de date bugzilla 2018-04-19T07:50:40Z
    	Date date = null;
    	try {
    		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
    		date = formatter.parse(dateBugzilla);
	 ...
	 static public String bugzillaStringDateToString(String dateBugzilla) {
    	
    	String dateStr = null;
		Date d = bugzillaStringDateToDate(dateBugzilla);
