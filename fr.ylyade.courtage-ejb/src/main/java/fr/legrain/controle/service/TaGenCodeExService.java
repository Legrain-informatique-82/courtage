...
import fr.ylyade.courtage.model.mapper.TaGenCodeExMapper;
...
//TODO A DECOMMENTER
			//TaInfoEntreprise taInfoEntreprise = entrepriseService.findInstance();
//			TaInfoEntreprise taInfoEntreprise = null;
			TaGenCodeEx taGenCodeEx = dao.findByCode(nomClass);
			String valeur=null;
			if(params!=null && params.size()>0){
				...
				//rajout yann pour ajout de prefixe sur le numPolice
				valeur=params.get("prefixe");
				if(valeur!=null) {
					taGenCodeEx.setPrefixe(valeur);
				}
				
				//rajout yann pour numPolice 
				valeur=params.get("PIB");
				if(valeur!=null) {
					taGenCodeEx.setCodeDocument(valeur);
				}
			}

			while (!nonAutorise) {
				compteur ++;
				//TODO A DECOMMENTER YLYADE		
				//nouveauCode = genereCodeExJPA(taGenCodeEx,compteur,nomClass,taInfoEntreprise.getCodexoInfoEntreprise());
				nouveauCode = genereCodeExJPA(taGenCodeEx,compteur,nomClass,"");
				nonAutorise = autoriseUtilisationCodeGenere(taGenCodeEx,nouveauCode,true);
			}
			if(!appelClientRiche()) {
				rentreCodeGenere(taGenCodeEx, nouveauCode, sessionInfo.getSessionID());
