	...
	public String genereCodeExJPA(TaGenCodeEx genCodeEx, int rajoutCompteur, String section, String exo) throws Exception {
		...
		if(genCodeEx!=null && genCodeEx.getValeurGenCode()!=null){
			try {
				...
				res="";
				//rajout yann pour prefixe
				if(genCodeEx.getPrefixe() != null) {
					res+=genCodeEx.getPrefixe();
				}
				regex+="([^{}]*)(\\{"+C_EXO+"\\})([^{}]*)";
				regex+="|([^{}]*)(\\{"+C_NUM+":?\\d?\\})([^{}]*)";
...
//										valeur = LibConversion.integerToString(LibConversion.stringToInteger(valeur)+1);					
			...
			//							res+=heure+":"+mn;
				...
				//rajout Yann numPolice PIB
				if(genCodeEx.getCodeDocument() != null && genCodeEx.getCodeDocument().equals("true")) {//SI C'EST UN PIB
					res += "Z"; //ON AJOUTE LA LETTRE Z
				}else if(genCodeEx.getCodeDocument() != null && genCodeEx.getCodeDocument().equals("false")) {//SI CE N'EST PAS UN PIB
					res += "S";//SI CE N'EST PAS UN PIB, LA LETTRE S
				}
			} catch (Exception e) {
